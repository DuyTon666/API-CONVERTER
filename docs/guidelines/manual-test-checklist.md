...
## 2026-07-20 — Verify DEF-04 sau khi cải thiện prompt AI-fix (TC-AIFIX-03 lặp lại)

Sau khi thêm chỉ dẫn rõ ràng chống mô tả chung chung vào `_build_batch_prompt()`
(`backend/services/ai_fix.py`), gọi trực tiếp `services.ai_fix.run()` qua Claude thật
(dùng `backend/.env` sẵn có) với 3 operation tổng hợp khác nhau cùng thiếu description
trong 1 batch — mô phỏng đúng tinh thần lỗi gốc của DEF-04: `createReopen` (POST
.../reopen), `deleteAttachment` (DELETE), `listComments` (GET list).

| TC ID | Kết quả thực tế |
| --- | --- |
| TC-AIFIX-03 (lặp lại) | ✅ Cả 3 operation nhận description riêng biệt, đúng nghiệp vụ: `createReopen` → "Mở lại ticket đã đóng, chuyển trạng thái ticket về trạng thái đang xử lý."; `deleteAttachment` → "Xóa tệp đính kèm khỏi ticket theo ID tệp đính kèm."; `listComments` → "Lấy danh sách tất cả bình luận thuộc ticket theo ID ticket." Không còn hiện tượng dùng chung 1 mô tả generic cho nhiều operation khác nhau như DEF-04 ghi nhận trước đó. |

**Giới hạn của lần verify này:** chỉ test với 3 operation tổng hợp (không phải đúng 5
operation gốc gây lỗi DEF-04 lúc phát hiện), 1 lần gọi AI duy nhất — đầu ra AI không
tất định 100%, có thể khác nếu chạy lại hoặc gặp operation phức tạp hơn. Coi là bằng
chứng mạnh cho thấy prompt mới hoạt động đúng hướng, không phải chứng minh tuyệt đối
đã hết bug vĩnh viễn cho mọi trường hợp tương lai.
...
| DEF-04 | Chất lượng AI-fix: khi 1 lượt fix nhiều lỗi "thiếu description" cùng lúc cho nhiều operation khác nhau trong 1 batch, AI sinh description chung chung/sai nghiệp vụ (vd `createReopen` → "Lấy danh sách tài nguyên." dù là API mở lại ticket; sai tương tự cho 4 operation khác). Khác với field schema (Phần A7/TC-AIFIX-02, nội dung AI sinh đúng) — do thiếu context riêng từng operation khi fix dồn 1 lượt                                                                                                                                     | TC-AIFIX-03 (2026-06-29)          | Chất lượng output AI, không phải bug logic                   | ✅ Đã fix (2026-07-20) — thêm chỉ dẫn rõ ràng vào `_build_batch_prompt()` (`backend/services/ai_fix.py`): bắt suy luận nghiệp vụ từ operationId/method/path, cấm mô tả generic kiểu "Lấy danh sách tài nguyên" khi không đúng, nhắc mỗi đoạn trong batch là 1 operation khác nhau. Verify end-to-end bằng 1 lần gọi Claude thật (xem TC-AIFIX-03 lặp lại, Phần B 2026-07-20) — cả 3 operation test (`createReopen`/`deleteAttachment`/`listComments`) nhận mô tả riêng biệt, đúng nghiệp vụ. Có test khoá nội dung prompt (`test_build_batch_prompt_warns_against_generic_description`, `backend/tests/test_ai_fix.py`) để tránh bị xoá nhầm sau này — nhưng chất lượng AI vẫn không tất định 100%, không đảm bảo tuyệt đối cho mọi trường hợp tương lai |
