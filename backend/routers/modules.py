...
# sync_operation_fields() (bundle_sync.py) — cùng cơ chế dual-write dùng chung
# với Form Editor/YAML thô. "accept_new": không đổi gì trong file (giá trị mới
# đã có sẵn từ lúc Phần 2 phát hiện conflict). Cả 2 case: xoá entry khỏi hàng đợi.
@router.post("/modules/manual-edit-conflicts/resolve")
def resolve_manual_edit_conflict(payload: dict = Body(...)):
    import yaml as _yaml
    from config import DIST_DIR
    from import_flow.config import REPORT_DIR
    from bundle_sync import Change, sync_operation_fields, _index_operation_files
...
def _run_import_job(job_id: str, module_filter: str | None = None) -> None:
    ...
    from manual_edit_conflicts import _scan_manual_edits, _resolve_manual_edits_after_import
