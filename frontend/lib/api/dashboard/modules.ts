...
export async function fetchManualEditConflicts(
  backend: string,
): Promise<ManualEditConflict[]> {
  return apiFetch<ManualEditConflict[]>(
    `${backend}/modules/manual-edit-conflicts`,
  );
}
...
export async function resolveManualEditConflict(
  ...
  await apiFetch(`${backend}/modules/manual-edit-conflicts/resolve`, {
    ...
    body: JSON.stringify({ kind, entityId, field, choice }),
  });
}
