...
import { methodTone, statusTone, type PastelTone } from "./theme";
...
function ResponseCard({
  ...
  return (
    <div className="border border-[#EAEAEA] rounded-lg overflow-hidden">
      ...
      >
        <span className={`text-sm font-bold px-2.5 py-1 rounded-full ${st.bg} ${st.text}`}>
          {status}
        </span>
        <span className="text-base text-gray-600 flex-1">{res.description}</span>
        <svg
...
export default function EndpointDetailDrawer({
  ...
  return (
    <div className="bg-white border border-[#EAEAEA] rounded-xl overflow-hidden">
      ...
      <div className="px-8 py-7 border-b border-[#F0F0EE]">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3 flex-wrap">
            <span className={`text-sm font-semibold tracking-wide px-3 py-1.5 rounded-full ${tone.bg} ${tone.text}`}>
              {op.method}
            </span>
            <code className="text-lg font-mono text-[#2F3437] break-all">{op.path}</code>
          </div>
          <button onClick={onClose} className="text-gray-300 hover:text-gray-500 text-2xl leading-none ml-4 shrink-0">✕</button>
        </div>
        {op.summary && <p className="mt-4 text-xl font-semibold text-[#2F3437]">{op.summary}</p>}
        {op.description && <p className="mt-2 text-base text-gray-500 whitespace-pre-wrap leading-relaxed">{op.description}</p>}
        {(op.tags.length > 0 || op.operationId) && (
          <div className="mt-4 flex items-center gap-2 flex-wrap">
            {op.tags.map((tag) => (
              <span key={tag} className="text-xs uppercase tracking-wide bg-[#F7F6F3] text-gray-500 px-2.5 py-1 rounded-full">{tag}</span>
            ))}
            {op.operationId && (
              <span className="text-sm font-mono text-gray-400">{op.operationId}</span>
            )}
          </div>
        )}
      </div>
      ...
      {parameters.length > 0 && (
        <div className="px-8 py-7 border-b border-[#F0F0EE]">
          <h3 className="text-sm font-semibold uppercase tracking-wider text-gray-400 mb-4">Parameters</h3>
          <table className="w-full text-sm border border-[#EAEAEA] rounded-lg overflow-hidden">
            ...
            <tbody>
              {parameters.map((p, i) => (
                <tr key={`${p.in}-${p.name}`} className={i % 2 === 0 ? "bg-white" : "bg-[#FBFBFA]"}>
                  <td className="px-4 py-3 font-mono text-[#2F3437] font-medium">{p.name}</td>
                  <td className="px-4 py-3">
                    <span className="px-2 py-0.5 bg-[#F7F6F3] text-gray-500 rounded">{p.in}</span>
                  </td>
      ...
      {requestBody && (
        <div className="px-8 py-7 border-b border-[#F0F0EE]">
          <div className="flex items-center gap-2 mb-4">
            <h3 className="text-sm font-semibold uppercase tracking-wider text-gray-400">Request Body</h3>
            {requestBody.required && <span className="text-sm text-[#9F2F2D]">required</span>}
          </div>
          {Object.entries(requestBody.content ?? {}).map(([mediaType, mediaObj]) => (
            <div key={mediaType}>
              <p className="text-sm font-mono text-gray-400 mb-3">{mediaType}</p>
      ...
      {Object.keys(responses).length > 0 && (
        <div className="px-8 py-7">
          <h3 className="text-sm font-semibold uppercase tracking-wider text-gray-400 mb-4">Responses</h3>
          <div className="space-y-4">
            {Object.entries(responses).map(([status, res]) => (
              <ResponseCard key={status} status={status} res={res} st={statusTone(status)} />
            ))}
          </div>
        </div>
      )}
